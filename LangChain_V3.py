...
if uploaded_files:
    ...
    prompt_template = """Given the context and the given question,provide a comprehensive answer in detail from the relevant information presented in the context.
           If you don't know the answer or question cannot be answered from the context, just reply with 'No relevant context in documents', don't try to make up an answer.
            Context: {context}
            Question: {question}
            Answer:"""
    ...
    rqa = RetrievalQA.from_chain_type(llm=ChatOpenAI(model_name="gpt-3.5-turbo-16k",temperature=0),
                                    chain_type="stuff",
                                    retriever=retriever,
                                    chain_type_kwargs={
                                    "prompt": PromptTemplate(template=prompt_template,input_variables=["context", "question"])},
    ...
    if submit_button: 
        ...
        if not(result['result']=='No relevant context in documents.' or result['result']=='No relevant context in documents'):
            file_name=get_file_name_with_extension(result['source_documents'][0].metadata['source'])
            ...
            page_num=result['source_documents'][0].metadata['page']
            page_num=page_num+1
